...
def _prepare_dict(d):
    ...
    for key in _sorted_keys(d, head_keys):
        value = d[key]
        # Filter None and empty strings (match Munki's behaviour)
        if value is None:
            continue
...
def load_pkginfo_yaml(f):
    ...
    Normalizes version-like keys that YAML may have parsed as numeric
    back to strings."""
...
def detect_munki_format(file_path):
    ...
    2. Content sniffing (``<?xml`` / ``<plist>`` → plist, ``---`` / ``key: value`` → yaml)
    3. Default: ``"plist"``
    """
